...
for c = 1:model.numcomponents
  ...
  root_pca{c} = model.rootfilters{ridx{c}}.wpca;
  ...
  for j = 1:numparts{c}
    ...
    part_pca{c,j} = model.partfilters{pidx{c,j}}.wpca;
...
pcax = size(model.pca_coeff, 1);
pcay = size(model.pca_coeff, 2);
...
for c = 1:model.numcomponents
    ...
    fprintf(f, '\t\t<HypothesisThresholdPCA>%.16fi</HypothesisThresholdPCA>\n', thresh(2*threshptr - 1));
